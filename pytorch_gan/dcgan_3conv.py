...
class DCGAN_G(nn.Module):
    ...
    def deconv_block_upsample(self, main, name, inf, of, s, bn=True):
        main.add_module(f'{name}-{of}.UpSample', nn.UpsamplingNearest2d(scale_factor=s))
        # main.add_module(f'{name}-{inf}.{of}.conv', nn.Conv2d(inf, of, 1, bias=False))
        main.add_module(f'{name}-{inf}.{of}.conv', nn.Conv2d(inf, of, 3, 1, 1, bias=False))
        main.add_module(f'{name}-{of}.batchnorm', nn.BatchNorm2d(of))
        main.add_module(f'{name}-{of}.relu', nn.ReLU(inplace=True))
    ...
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0):
        ...
        main = nn.Sequential()
        self.deconv_block_upsample(main, 'initial_1', nz, cngf, 2)
        self.deconv_block_upsample(main, 'initial_2', cngf, cngf, 2)
        # self.deconv_block(main, 'initial', nz, cngf, 4, 1, 0)

        csize, cndf = 4, cngf
        while csize < isize//2:
            self.deconv_block_upsample(main, 'pyramid', cngf, cngf//2, 2)
            # self.deconv_block(main, 'pyramid', cngf, cngf//2, 4, 2, 1)
            cngf //= 2; csize *= 2

        for t in range(n_extra_layers):
            # self.deconv_block(main, f'extra-{t}', cngf, cngf, 1, 1, 0)
        ...
        main.add_module(f'final.{cngf}.UpSample', nn.UpsamplingNearest2d(scale_factor=2))
        # main.add_module(f'final.{cngf}-{nc}.conv', nn.Conv2d(cngf, nc, 1, bias=False))
        main.add_module(f'final.{cngf}-{nc}.conv', nn.Conv2d(cngf, nc, 3, 1, 1, bias=False))
