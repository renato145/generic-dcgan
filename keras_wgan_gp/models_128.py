...
from keras.layers import Input, Dense, Reshape, Flatten, Activation, GlobalAvgPool2D, RepeatVector
...
def make_discriminator():
    ...
    y = conv_layer(y, 512, act='lrelu', bn=False)
    y = conv_layer(y, 512, act='lrelu', bn=False)
    y = conv_layer(y, 1024, act='lrelu', bn=False)
    y = GlobalAvgPool2D()(y)
    y = dense_layer(y, 512, act='lrelu', bn=False)
...
def make_generator():
    x = Input(shape=(128,))
    y = dense_layer(x, 512)
    y = dense_layer(y, 1024)
    y = RepeatVector(4)(y)
    y = Reshape((2,2,1024))(y)
    y = conv_layer(y, 512, transpose=True)
    y = conv_layer(y, 512, transpose=True)
